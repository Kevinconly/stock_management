...
route.post('/login', (req, res)=>{
    ...
    conn.query(check_email_in_db, [user_email], async(err, result)=>{
        if(err){
            res.status(500).json({message:'Failed to find Your Email'})
        }else if(result.length <0){
            res.status(403).json({message:"Email not registred yet"})
        }else{
            const user = result[0]
            const is_password_match = await bcrypt.compare(user_password, user.user_password)
            if(is_password_match === false){
                res.status(401).json({message:"Wrong password"})
            }else{
                const token = jwt.sign( user.user_id, SECRETE, {expiresIn:"1d"})
            }
        }
    })
})
